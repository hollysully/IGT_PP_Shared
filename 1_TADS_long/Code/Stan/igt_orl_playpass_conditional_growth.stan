...
transformed data {
  int D_int = D_end[4]; // number of intercept params
  int D_slope = D-D_end[4]; // number of slope params
}

parameters {
  vector[D_int] gamma0;
  vector[D_slope] gamma1;
...
transformed parameters {
  ...
  for (p in 1:4) {
    ...
    for (s in 1:S) {
      ...
      beta[p,s][,2] = to_matrix(X[,s,D_start[p+4]:D_end[p+4]]) * gamma1[(D_start[p+4]-D_int):(D_end[p+4]-D_int)] + to_vector(beta_tilde[p][2,]); 
    }
  }
