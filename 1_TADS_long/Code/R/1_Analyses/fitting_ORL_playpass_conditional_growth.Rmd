...
ggplot(draws_df, aes(x = .value, y = group)) +
  ...
  stat_pointinterval(.width=c(.8, .95), point_interval = "median_hdi") +
...
if (FIT_CONDITIONAL) {
  ...
  pred_growth_summary <- pred_growth %>%
    ...
    summarise(
      lower = HDIofMCMC(pred, 0.50)[1],  # lower 50% hdi
      upper = HDIofMCMC(pred, 0.50)[2],  # upper 50% hdi
  ...
  ggplot(pred_growth_summary, aes(
    x = session,
    fill = group,
    color = group,
    shape = group,
    ...
    scale_color_brewer("50% CI", palette = "Set1") +
    scale_shape_discrete("50% CI") +
...
if (!FIT_CONDITIONAL) {
  ...
  pred_growth_summary <- pred_growth %>%
    ...
    summarise(
      q025 = HDIofMCMC(pred, .95)[1],
      q10  = HDIofMCMC(pred, .80)[1],
      q25  = HDIofMCMC(pred, .50)[1],
      q50  = quantile(pred, 0.50),
      q75  = HDIofMCMC(pred, .50)[2],
      q90  = HDIofMCMC(pred, .80)[2],
      q975 = HDIofMCMC(pred, .95)[2],
      .groups = "drop"
    ) %>%
    mutate(
      par = factor(
