...
for(i in 1:iterations){
  ...
  R[i] = rnorm(1, mean = 0, sd = .05)
  corr[,,i] = rethinking::rlkjcorr(1, K = 2, eta = 1)
  VC = diag(sigma_U[i,]) %*% corr[,,i] %*% diag(sigma_U[i,])
  
  beta[,,i] = mvrnorm(iterations, mu = c(gamma00[i], gamma10[i]), Sigma = VC) + R[i]
}
...
```{r, eval = T}
# -----------------------------------------------------------------------------------
RL_model = stan_model(here("1_HLCM", "Code", "Stan", "RL_HLCM_Model.stan")) # Compile model
...
saveRDS(timestamp, here("1_HLCM", "Data", "2_Fitted",
                        paste0("RL_HLCM_", condition, "_Timestamp.RDS")))
saveRDS(RL_fit, here("1_HLCM", "Data", "2_Fitted",
                     paste0("RL_HLCM_", condition, "_Fit.RDS")))

# -----------------------------------------------------------------------------------
# save posteriors
RL_posteriors = extract(RL_fit)
saveRDS(RL_posteriors, here("1_HLCM", "Data", "2_Fitted",
                            paste0("RL_HLCM_", condition, "_posteriors.RDS")))
...
saveRDS(RL_rhats, here("1_HLCM", "Data", "2_Fitted",
                       paste0("RL_HLCM_", condition, "_rhats.RDS")))
```
