...
```{r}
# TEST
# RL_Characteristics = readRDS(here("1_HLCM", "Data", "2_Fitted", "Test", "RL_Characteristics.RDS"))
# RL_Simulations     = readRDS(here("1_HLCM", "Data", "2_Fitted", "Test", "RL_Simulations.RDS"))
# RL_Diagnostics     = readRDS(here("1_HLCM", "Data", "2_Fitted", "Test", "RL_Diagnostics.RDS"))
# 
# HLCM_Characteristics = readRDS(here("1_HLCM", "Data", "2_Fitted", "Test", "HLCM_Characteristics.RDS"))
# HLCM_Simulations     = readRDS(here("1_HLCM", "Data", "2_Fitted", "Test", "HLCM_Simulations.RDS"))
# HLCM_Diagnostics     = readRDS(here("1_HLCM", "Data", "2_Fitted", "Test", "HLCM_Diagnostics.RDS"))

# SIMULATION
RL_Characteristics = readRDS(here("1_HLCM", "Data", "2_Fitted", "RL_Characteristics.RDS"))
RL_Simulations     = readRDS(here("1_HLCM", "Data", "2_Fitted", "LoB_LoR", "RL_Simulation_1.RDS"))
RL_Diagnostics     = readRDS(here("1_HLCM", "Data", "2_Fitted", "RL_Diagnostics.RDS"))

HLCM_Characteristics = readRDS(here("1_HLCM", "Data", "2_Fitted", "HLCM_Characteristics.RDS"))
HLCM_Simulations     = readRDS(here("1_HLCM", "Data", "2_Fitted", "HLCM_Simulations.RDS"))
HLCM_Diagnostics     = readRDS(here("1_HLCM", "Data", "2_Fitted", "HLCM_Diagnostics.RDS"))

Total_Duration = readRDS(here("1_HLCM", "Data", "TimeStamps.RDS"))
```


## Combine Simulations
```{r}
simulations = bind_rows(mutate(RL_Simulations, model = "RL"),
                        mutate(HLCM_Simulations, model = "HLCM"))
```
...
summarized_durations = bind_rows(mutate(RL_Characteristics, model = "RL"), 
                                 mutate(HLCM_Characteristics, model = "HLCM")) %>%
  group_by(model) %>% 
  summarise(total = round(sum(as.numeric(duration, units = "hours")), 2),
            avg = round(mean(as.numeric(duration, units = "mins")), 2)) %>% 
  pivot_wider(names_from = "model", values_from = c("total", "avg"))

bind_rows(mutate(RL_Characteristics, model = "RL"), 
          mutate(HLCM_Characteristics, model = "HLCM")) %>% 
  group_by(model, condition) %>% 
  mutate(mean_duration = mean(duration)) %>% 
  mutate(model = factor(model, levels = c("RL", "HLCM"), ordered = T),
  ...
  ggplot(aes(x = duration, fill = model)) +
    # GEOMS
    geom_histogram(color = "black", linewidth = .75, bins = 50) +
    # geom_text_npc(aes(npcx = "right", npcy = "top", label = paste(round(mean_duration, 2), "mins")), 
    #               size = 4, check_overlap = T) +
    geom_hline(yintercept = 0) +
    geom_vline(aes(xintercept = mean_duration, color = model),
    ...
    labs(x = "Fit Duration (Mins)", y = "Frequency",
         caption = paste0("RL Duration: Total = ", summarized_durations$total_RL, " hours, Mean = ",
                          summarized_durations$avg_RL, " mins \n",
                          "HLCM Duration: Total = ", summarized_durations$total_HLCM, " hours, Mean = ",
    ...
    facet_rep_grid(condition_lab~model, scales = "free",
...
simulations %>% 
  ...
  group_by(model, condition, parameter) %>% 
  ...
  mutate(model = factor(model, levels = c("RL", "HLCM"), ordered = T),
         condition_lab = factor(condition, levels = c("LoB_LoR", "HiB_LoR", "LoB_HiR", "HiB_HiR"),
                                labels = c(expression("Lo~\u03B2~Lo~r"),
                                           ...
                                           expression("Lo~\u03B2~Hi~r"),
                                           expression("Hi~\u03B2~Hi~r")))) %>% 
  ggplot(aes(x = posterior_mean, fill = model)) +
    # GEOMS
    geom_histogram(color = "black", linewidth = .75, bins = 50) +
    geom_hline(yintercept = 0) +
    geom_vline(aes(xintercept = true_value), color = "red2", linewidth = .75) +
    geom_vline(aes(xintercept = mean_posterior_mean, color = model), 
               linewidth = .75, linetype = "dashed", show.legend = F) +
    # SCALES
  ...
  mutate(model = factor(model, levels = c("RL", "HLCM"), ordered = T),
         condition_lab = factor(condition, levels = c("LoB_LoR", "HiB_LoR", "LoB_HiR", "HiB_HiR"),
                                labels = c(expression("Lo~\u03B2~Lo~r"),
                                           ...
                                           expression("Lo~\u03B2~Hi~r"),
                                           expression("Hi~\u03B2~Hi~r")))) %>% 
  ggplot(aes(x = percentile, fill = model)) +
    # GEOMS
    geom_histogram(color = "black", linewidth = .75, bins = 50) +
    geom_vline(xintercept = .5, color = "red2", linewidth = .75) +
    geom_vline(aes(xintercept = mean_percentile, color = model),
               linewidth = .75, linetype = "dashed", show.legend = F) +
    geom_hline(yintercept = 0) +
    # SCALES
...
simulations %>% 
  filter(parameter_type == "group") %>%
  group_by(condition, parameter, model) %>% 
  ...
  mutate(model = factor(model, levels = c("RL", "HLCM"), ordered = T),
         condition_lab = factor(condition, levels = c("LoB_LoR", "HiB_LoR", "LoB_HiR", "HiB_HiR"),
                                labels = c(expression("Lo~\u03B2~Lo~r"),
                                           ...
                                           expression("Lo~\u03B2~Hi~r"),
                                           expression("Hi~\u03B2~Hi~r")))) %>% 
  ggplot(aes(x = parameter, y = true_in_HDI95, fill = model)) +
    # GEOMS
    ...
    theme(axis.text.x = element_text(color = "black", size = 12, angle = 90, vjust = .5),
          ...
          panel.background = element_rect(fill = "transparent", colour = "transparent"),
          plot.background = element_rect(fill = "transparent", color = "transparent")) +
    facet_rep_grid(condition_lab~model, scales = "free", labeller = label_parsed)
```
...
simulations %>% 
  ...
  group_by(simulation, condition, model, parameter) %>% 
  ...
  mutate(model = factor(model, levels = c("RL", "HLCM"), ordered = T),
         condition_lab = factor(condition, levels = c("LoB_LoR", "HiB_LoR", "LoB_HiR", "HiB_HiR"),
                                labels = c(expression("Lo~\u03B2~Lo~r"),
                                           ...
                                           expression("Lo~\u03B2~Hi~r"),
                                           expression("Hi~\u03B2~Hi~r")))) %>% 
  ggplot(aes(x = r, fill = model)) +
    # GEOMS
    geom_histogram(color = "black", linewidth = .75, bins = 50) +
    geom_hline(yintercept = 0) +
    geom_vline(aes(xintercept = mean_r, color = model),
               linewidth = .75, linetype = "dashed", show.legend = F) +
