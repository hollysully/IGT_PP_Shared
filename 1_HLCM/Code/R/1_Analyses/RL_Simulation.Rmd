...
# -----------------------------------------------------------------------------------
# COMPILE MODELS
RL_model     = cmdstan_model(here("1_HLCM", "Code", "Stan", "RL_Model.stan")) # Compile model
...
foreach(condition = conditions) %:% # LOOP THROUGH CONDITIONS
  foreach(sim = 1:n_simulations) %dopar% {
    ...
    stan_data = list(N = N,
                     ...
                     choice = choice,
                     outcome = outcome)
    saveRDS(stan_data, here::here("1_HLCM", "Data", "1_Stan", condition,
    ...
    start_time = Sys.time()
    for(s in 1:S){
      RL_fit = RL_model$sample(data = list(N = stan_data$N,
                                           T = stan_data$T,
      ...
      unbounded_A_posterior[,s,] = par_from_draws(RL_fit, "theta")$theta
      A_posterior[,s,] = par_from_draws(RL_fit, "A")$A
      RL_Diagnostics = RL_fit$summary(variables = c("mu_theta", "mu_A", "sigma_U", "theta", "A")) %>% 
        dplyr::select(parameter = variable, rhat, ess_bulk, ess_tail) %>% 
        mutate(condition = condition, session = s, model = "RL", simulation = sim) %>% 
    ...
    ESS = bind_rows(dplyr::select(summary(growth_fit)$fixed,
                                  ess_bulk = Bulk_ESS, ess_tail = Tail_ESS),
                    dplyr::select(summary(growth_fit)$random$ID,
                                  ess_bulk = Bulk_ESS, ess_tail = Tail_ESS),
                    dplyr::select(summary(growth_fit)$spec_pars,
                                  ess_bulk = Bulk_ESS, ess_tail = Tail_ESS)) %>% 
      mutate(parameter = rownames(.),
             parameter = case_when(parameter == "session" ~
                                     "b_session",
                                   parameter == "sd(Intercept)" ~
                                     "sd_ID__Intercept",
                                   parameter == "sd(session)" ~
                                     "sd_ID__session",
                                   parameter == "cor(Intercept,session)" ~
                                     "cor_ID__Intercept__session",
                                   T ~ parameter))
    ...
    RL_Diagnostics = RL_growth_diagnostics %>% 
      ...
      dplyr::select(condition, simulation, model, session, parameter, rhat, ess_bulk, ess_tail) %>% 
      bind_rows(RL_Diagnostics)
    ...
    cur_parameter = "residual_SD"
    cur_true_parameter = residual_sd
    cur_posterior = par_from_draws(HLCM_fit, "R")$R
    cur_hdi = hBayesDM::HDIofMCMC(cur_posterior)
    HLCM_Simulations =
      data.frame(condition = condition, simulation = sim, parameter = cur_parameter,
                 parameter_type = "group", session = 0,
    ...
    # -----------------------------------------------------------------------------------
    # SAVE DIAGNOSTIC DATA
    HLCM_Diagnostics = HLCM_fit$summary(variables = c("gamma00", "gamma10", "R", "sigma_U",
                                                      "beta0", "beta1", "theta", "A","R_theta")) %>% 
      dplyr::select(parameter = variable, rhat, ess_bulk, ess_tail) %>%
      filter(!(parameter %in% c("R_theta[1,1]", "R_theta[1,2]", "R_theta[2,2]"))) %>% 
      mutate(condition = condition, simulation = sim,
