...
// Declare transformed parameters
  ...
  for(s in 1:S){    // Loop over sessions
    for(i in 1:N){  // Loop over subjects - This is structured the same as the OG joint retest model such that Arew,
      ...
      Arew[i,s] = Phi_approx(mu_p[s,1] + Arew_pr[s,i]*sigma_Arew[s]);
      Apun[i,s] = Phi_approx(mu_p[s,2] + Apun_pr[s,i]*sigma_Apun[s]);
      K[i,s]    = Phi_approx(mu_p[s,3] + K_pr[s,i]*sigma_K[s]) * 5;
      betaF[i,s] = mu_p[s,4] + betaF_pr[s,i]*sigma_betaF[s];
      betaP[i,s] = mu_p[s,5] + betaP_pr[s,i]*sigma_betaP[s];
    }
  }
}
