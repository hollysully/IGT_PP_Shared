...
// Declare parameters
  ...
  real mu_cow[2];             // mu of carryover weights
  real<lower=0> sigma_cow[2]; // SD of carryover weights
  ...
  // Subject-level, untransformed carryover weight (cow)
  vector[N] cow_pr;
...
// Declare transformed parameters
  ...
  matrix[N,S] betaF;
  matrix[N,S] betaP;
  matrix[N,2] cow; // Transformed carryover weight
  ...
  for(i in 1:N){  // Loop over subjects
    cow[i,1] = Phi_approx(mu_cow[1] + cow_pr[i]*sigma_cow[1]);
    cow[i,2] = Phi_approx(mu_cow[1] + mu_cow[2] + cow_pr[i]*sigma_cow[2]);
    for(s in 1:S){    // Loop over sessions
      ...
      betaF[i,s] = mu_p[s,4] + betaF_tilde[s,i];
      betaP[i,s] = mu_p[s,5] + betaP_tilde[s,i];
    }
  }
}
...
model {
  ...
  // Hyperparameters for carryover weight
  mu_cow    ~ normal(0, 1);
  sigma_cow ~ normal(0, 0.2);
  ...
  for (i in 1:N) {         // Loop through individual participants
    for (s in 1:S) {       // Loop though sessions for participant i
      if (Tsubj[i,s] > 0) {    // If we have data for participant i on session s, run through RL algorithm
        ...
        K_tr = pow(3, K[i,s]) - 1;
        if(s == 1){ // No carryover weight for session 1
        ...
        } else { // Use carryover weight for session 2
          for (pp in 1:2) {  // Looping over play/pass columns and assigning each 0 to each card
            ev[:,pp] *= cow[i,1];
            ef[:,pp] *= cow[i,2];
            pers[:,pp] = rep_vector(0,4);
