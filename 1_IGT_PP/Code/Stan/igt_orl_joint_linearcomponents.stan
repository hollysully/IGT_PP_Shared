...
// Calculate transformed parameters
  ...
  // Here, I'm constructing the linear component of the parameters before any transformations
    // This could be embedded in the next loop, but for now, this helps to understand this as a linear model
  for(s in 1:S){
    for(i in 1:N){
      // For each linear model, I'm adding the group-level mu (mu_p) for session s, a random-intercept term which is
        ...
        // element of the tilde parameter should be the overall individual-subject mu - I think this would be equivalent
        // to taking the average of session 1 and session 2 parameters for individual subjects
      linear_Arew[s,i]  = mu_p[s,1] + Arew_tilde[1,i] + Arew_tilde[2,i]*(s-1);   // subtracting 1 from s makes 0 the first timepoint
      linear_Apun[s,i]  = mu_p[s,2] + Apun_tilde[1,i] + Apun_tilde[2,i]*(s-1);   // subtracting 1 from s makes 0 the first timepoint
      linear_K[s,i]     = mu_p[s,3] + K_tilde[1,i] + K_tilde[2,i]*(s-1);         // subtracting 1 from s makes 0 the first timepoint
      linear_betaF[s,i] = mu_p[s,4] + betaF_tilde[1,i] + betaF_tilde[2,i]*(s-1); // subtracting 1 from s makes 0 the first timepoint
      linear_betaP[s,i] = mu_p[s,5] + betaP_tilde[1,i] + betaP_tilde[2,i]*(s-1); // subtracting 1 from s makes 0 the first timepoint
