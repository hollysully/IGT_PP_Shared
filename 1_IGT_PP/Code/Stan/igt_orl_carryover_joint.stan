...
// Declare parameters
  ...
  real mu_cow;             // mu of carryover weights
  real<lower=0> sigma_cow; // SD of carryover weights
...
// Declare transformed parameters
  ...
  matrix[N,S] betaF;
  matrix[N,S] betaP;
  vector[N] cow; // Transformed carryover weight
  ...
  for(i in 1:N){  // Loop over subjects
    cow[i] = Phi_approx(mu_cow + cow_pr[i]*sigma_cow);
