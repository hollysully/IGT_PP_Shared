...
c = .2 # constant for accentuated simulations
parameters = data.frame(Arew = orl_posteriors$mu_Arew,
  ...
  mutate(# Databased simulations ########################################################
         # Session 1 - baseline
         session_1 = session_1,
         # Change in parameters except betaB - main effect of session on A+
         ME_Arew = case_when(parameter == "betaB" ~ session_1, T ~ session_2),
         # Change in parameters except Arew - main effect of session on BetaB
         ME_betaB = case_when(parameter == "Arew" ~ session_1, T ~ session_2),
         # Session 2 - interaction
         session_2 = session_2,
         # Accentuated simulations ######################################################
         BL = case_when(parameter == "Arew" ~ .25, parameter == "Apun" ~ .25,
                        parameter == "betaF" ~ 2.5, parameter == "betaB" ~ 2),
         
         loArew = case_when(parameter == "Arew" ~ BL - c, T ~ BL),
         hiArew = case_when(parameter == "Arew" ~ BL + c, T ~ BL),
         
         loApun = case_when(parameter == "Apun" ~ BL - c, T ~ BL),
         hiApun = case_when(parameter == "Apun" ~ BL + c, T ~ BL),
         
         negbetaF = case_when(parameter == "betaF" ~ -BL, T ~ BL),
         lobetaF = case_when(parameter == "betaF" ~ BL - (c*10), T ~ BL),
         hibetaF = case_when(parameter == "betaF" ~ BL + (c*10), T ~ BL),
         
         negbetaB = case_when(parameter == "betaB" ~ -BL, T ~ BL),
         lobetaB = case_when(parameter == "betaB" ~ BL - (c*5), T ~ BL),
         hibetaB = case_when(parameter == "betaB" ~ BL + (c*5), T ~ BL),
         
         loArew_hibetaB = case_when(parameter == "Arew" ~ BL - c,
                                    parameter == "betaB" ~ BL + (c*5), 
                                    T ~ BL))

#------------------------------------------------------------------------------
# RUN SIMULATIONS
# Simulation of observed changes in Arew & betaB with interaction
# databased_simulations = bind_rows(simulate_orl(parameters, "session_1"),
#                                   simulate_orl(parameters, "ME_Arew"),
#                                   simulate_orl(parameters, "ME_betaB"),
#                                   simulate_orl(parameters, "session_2"))
# saveRDS(databased_simulations, here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_Bias",
#                                     "databased_simulations.rds"))

#------------------------------------------------------------------------------
# Simulation of accentuated parameters
accentuated_simulations = bind_rows(simulate_orl(parameters, "BL"),
                                    simulate_orl(parameters, "loArew"),
                                    simulate_orl(parameters, "hiArew"),
                                    simulate_orl(parameters, "loApun"),
                                    simulate_orl(parameters, "hiApun"),
                                    simulate_orl(parameters, "negbetaF"),
                                    simulate_orl(parameters, "lobetaF"),
                                    simulate_orl(parameters, "hibetaF"),
                                    simulate_orl(parameters, "negbetaB"),
                                    simulate_orl(parameters, "lobetaB"),
                                    simulate_orl(parameters, "hibetaB"),
                                    simulate_orl(parameters, "loArew_hibetaB"))
saveRDS(accentuated_simulations, here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_Bias",
                                       "accentuated_simulations.rds"))
```



```{r}
databased_simulations = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_Bias",
                                     "databased_simulations.rds"))
accentuated_simulations = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_Bias",
                                       "accentuated_simulations.rds"))
```
...
## Arew Accentuated Simulation
```{r, eval = T}
labs = c("BL",
         expression(paste("\u0394",italic(A),"+",sep="")))

tiff(here("1_IGT_PP", "Figs_Tables", "Manuscript_Plots",
          "Accentuated Arew Simulations.tiff"),
     width = 6.5, height = 2.5, units = "in", res = 300)

  # data
  accentuated_simulations %>%
    filter(simulation %in% c("BL", "loArew")) %>% 
    select(everything(), deck = card) %>% 
    mutate(choice = case_when(choice == 2 ~ 0, T ~ choice),
           simulation = factor(simulation,
                               levels = c("BL", "loArew"))) %>% 
...
## BetaB Accentuated Simulation
```{r, eval = T}
labs = c("BL",
         expression(paste("\u0394","\u03B2",italic(b),sep="")))

tiff(here("1_IGT_PP", "Figs_Tables", "Manuscript_Plots",
          "Accentuated BetaB Simulations.tiff"),
     width = 6.5, height = 2.5, units = "in", res = 300)

  # data
  accentuated_simulations %>%
    filter(simulation %in% c("BL", "hibetaB")) %>% 
    select(everything(), deck = card) %>% 
    mutate(choice = case_when(choice == 2 ~ 0, T ~ choice),
           simulation = factor(simulation,
                               levels = c("BL", "hibetaB"))) %>% 
...
## BetaF Accentuated Simulation
```{r, eval = T}
labs = c("BL",
         expression(paste("-","\u03B2",italic(f),sep="")),
         expression(paste("Lo ","\u03B2",italic(f),sep="")),
         expression(paste("Hi ","\u03B2",italic(f),sep="")))

tiff(here("1_IGT_PP", "Figs_Tables", "Manuscript_Plots",
          "Accentuated BetaF Simulations.tiff"),
     width = 6.5, height = 2.5, units = "in", res = 300)

  # data
  accentuated_simulations %>%
    filter(simulation %in% c("BL", "negbetaF", "lobetaF", "hibetaF")) %>% 
    select(everything(), deck = card) %>% 
    mutate(choice = case_when(choice == 2 ~ 0, T ~ choice),
           simulation = factor(simulation, ordered = T,
                               levels = c("BL", "negbetaF", "lobetaF", "hibetaF"),
                               labels = c("Baseline",
                                          expression(paste0("-","\u03B2",italic(f))),
                                          expression(paste0("Lo ","\u03B2",italic(f))),
                                          expression(paste0("Hi ","\u03B2",italic(f)))))) %>% 
    group_by(simulation, card_trial, deck) %>% 
    reframe(proportion = mean(choice)) %>% 
    group_by(simulation, deck) %>% 
    arrange(card_trial) %>% 
