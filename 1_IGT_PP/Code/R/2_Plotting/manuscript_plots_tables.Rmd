...
## Plot All
```{r, eval = F}
all_choice_proportions = ind_choice_proportions %>% 
    left_join(data.frame(IDs)) %>% 
    mutate(id = paste0("ID #", subjID)) %>%
    select(everything(),
           prob = prob_mean, lower = lower95, upper = upper95,
...
batches = list(unique(all_choice_proportions$id)[1:5],
               unique(all_choice_proportions$id)[6:10],
               unique(all_choice_proportions$id)[11:15],
               unique(all_choice_proportions$id)[16:20],
               unique(all_choice_proportions$id)[21:25],
               unique(all_choice_proportions$id)[26:30],
               unique(all_choice_proportions$id)[31:35],
               unique(all_choice_proportions$id)[36:40],
               unique(all_choice_proportions$id)[41:45],
               unique(all_choice_proportions$id)[46:49])

for(i in batches){
  plot = all_choice_proportions %>%
      filter(str_detect(id, paste(i, collapse = "|"))) %>% 
      # plotting
      ggplot(aes(x = binC-2.5)) +
        # geoms
        ...
        facet_rep_grid(id~card,
                       labeller = labeller(card = c("1" = "Deck A", "2" = "Deck B",
                                                    "3" = "Deck C", "4" = "Deck D")))
  tiff(here("1_IGT_PP", "Figs_Tables", "Manuscript_Plots",
            paste(paste(i, collapse = " & "), "PPCs.tiff")),
