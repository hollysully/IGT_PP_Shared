...
library(ggh4x)
library(hypergeo)
source(here("1_IGT_PP", "Code", "R", "1_Analyses", "JH_plausible_correlations.R"))
...
for(param in c("Arew",
               "Apun",
               "K",
               "betaF", "betaP" # these do not get corrected because they're normally distributed, but I include them here to see that they don't change meaningfully
               )){
...
corrected_iterative_corrs %>% 
  ...
  plotHDI(.$correlation, binSize = 50) +
...
blups %>% 
  ...
  mutate(parameter = factor(parameter,
                            ...
                            labels = c(expression(paste(italic(A), "+", sep = "")),
                                       ...
                                       expression(italic(K))))) %>%
  # plotting
  ggplot(aes(x = blup_session1, y = blup_session2)) +
    # add geoms
    geom_point(shape = 1) +
