...
stan_data$Tsubj = stan_data$Tsubj[,1]
stan_data$choice = stan_data$choice[,,1]
stan_data$card = stan_data$card[,,1]
stan_data$outcome = stan_data$outcome[,,1]
stan_data$sign = stan_data$sign[,,1]
```
...
saveRDS(orl_fit, here("1_IGT_PP", "Data", "2_Fitted", "Sess1_ORL", "Sess1_ORL_fit.rds"))
```



```{r}
orl_fit = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "Sess1_ORL", "Sess1_ORL_fit.rds"))
...
saveRDS(orl_posteriors, here("1_IGT_PP", "Data", "2_Fitted", 
                             "Sess1_ORL", "Sess1_ORL_posteriors.rds"))
```
...
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Sess1_ORL", "Sess1_ORL_group_traceplots.tiff"),
...
for(i in person_parameters){
  tiff(here("1_IGT_PP", "Figs_Tables", 
            "Sess1_ORL", "Sess1_ORL_", i, "_traceplots.tiff", sep = ""),
       width = 35, height = 25, units = "cm", res = 300)
    traceplot(orl_fit,
              pars = i
              ) %>% print()
...
```{r, eval = T}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Sess1_ORL", "Sess1_ORL_posterior_means.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Sess1_ORL", "Sess1_ORL_posterior_distributions.tiff"),
