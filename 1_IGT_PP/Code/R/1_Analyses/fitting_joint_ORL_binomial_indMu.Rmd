...
for(scale in scales){
  ...
  saveRDS(orl_fit, here("1_IGT_PP", "Data", "2_Fitted", paste("igt_", scale, "_orl_binomial_joint_indMu.rds", sep = "")))
}
```
...
```{r}
orl_fit <- readRDS(here("1_IGT_PP", "Data", "2_Fitted", "igt_selfreport_orl_binomial_joint_indMu.rds"))
...
## Traceplots
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", "Joint_ORL_Binomial",
          "orl_pp_binomial_joint_indMu_group_traceplots.tiff"),
...
for(i in labels){
  tiff(here("1_IGT_PP", "Figs_Tables", "Joint_ORL_Binomial",
            paste("orl_pp_binomial_joint_indMu_", i, "_traceplots.tiff", sep = "")),
       width = 35, height = 25, units = "cm", res = 300)
    traceplot(orl_fit,
              pars = i
              ) %>% print()
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", "Joint_ORL_Binomial",
          "orl_pp_binomial_joint_indMu_posterior_means.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
tiff(here("1_IGT_PP", "Figs_Tables", "Joint_ORL_Binomial",
          "orl_pp_binomial_joint_indMu_posterior_distributions.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
```{r}
selfreport_models = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "self_report_models.RDS"))
selfreport = readRDS(here("1_IGT_PP", "Data", "1_Preprocessed", "stan_ready_binomial_selfreport.rds"))
```
