...
# Check Model
```{r, eval = F}
model_code = cmdstan_model(here("1_IGT_PP", "Code", "Stan", "igt_orl_carryover_evef_joint.stan"), compile = F)
...
```{r, eval = F}
# Compile model
orl_model = stan_model(here("1_IGT_PP", "Code", "Stan", "igt_orl_carryover_evef_joint.stan"))
...
saveRDS(orl_fit, here("1_IGT_PP", "Data", "2_Fitted", "ORL_Carryover_Joint",
                      "orl_pp_carryover_evef_joint_fit.rds"))
```



```{r}
orl_fit = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "ORL_Carryover_Joint",
                       "orl_pp_carryover_evef_joint_fit.rds"))
...
# saveRDS(orl_posteriors, here("1_IGT_PP", "Data", "2_Fitted", "ORL_Carryover_Joint",
#                              "orl_pp_carryover_evef_joint_posteriors.rds"))
orl_posteriors = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "ORL_Carryover_Joint",
                              "orl_pp_carryover_evef_joint_posteriors.rds"))


focal_parameters <- c(
  "mu_Arew", "mu_Apun", "mu_K", "mu_betaF", "mu_betaP", "mu_cow",
  "sigma_Arew", "sigma_Apun", "sigma_K", "sigma_betaF", "sigma_betaP", "sigma_cow",
  "Arew", "Apun", "K", "betaF", "betaP", "cow",
...
# saveRDS(rhats, here("1_IGT_PP", "Data", "2_Fitted", "ORL_Carryover_Joint",
#                     "orl_pp_carryover_evef_joint_rhats.rds"))

rhats = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "ORL_Carryover_Joint",
                     "orl_pp_carryover_evef_joint_rhats.rds"))
```
...
## Traceplots
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", "ORL_Carryover_Joint",
          "orl_pp_carryover_evef_joint_group_traceplots.tiff"),
...
for(i in person_parameters){
  tiff(here("1_IGT_PP", "Figs_Tables", "ORL_Carryover_Joint",
            paste("orl_pp_carryover_evef_joint_", i, "_traceplots.tiff", sep = "")),
       width = 35, height = 25, units = "cm", res = 300)
    traceplot(orl_fit,
              pars = i
              ) %>% print()
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", "ORL_Carryover_Joint",
          "orl_pp_carryover_evef_joint_posterior_means.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", "ORL_Carryover_Joint",
          "orl_pp_carryover_evef_joint_posterior_distributions.tiff"),
     width = 30, height = 20, units = "cm", res = 300)
  par(mfrow = c(5, 6))
  # Mus & Sigmas
  for(s in 1:2){
    for(i in grep("R_|cow", group_parameters, value = T, invert = T)){
  ...
  for(i in grep("cow", group_parameters, value = T)){
...
## Posterior Means
```{r}
apply(orl_posteriors$R_Arew, c(2,3), mean)
apply(orl_posteriors$R_Arew, c(2,3), HDIofMCMC)
apply(orl_posteriors$R_Apun, c(2,3), mean)
apply(orl_posteriors$R_Apun, c(2,3), HDIofMCMC)
apply(orl_posteriors$R_K, c(2,3), mean)
apply(orl_posteriors$R_K, c(2,3), HDIofMCMC)
apply(orl_posteriors$R_betaF, c(2,3), mean)
apply(orl_posteriors$R_betaF, c(2,3), HDIofMCMC)
apply(orl_posteriors$R_betaP, c(2,3), mean)
apply(orl_posteriors$R_betaP, c(2,3), HDIofMCMC)
```
...
correlations %>% 
  ...
  write.csv(here("1_IGT_PP", "Figs_Tables", "ORL_Carryover_Joint", "session_orl_sr_correlations.csv"))
