...
# Check Model
```{r, eval = T}
model_code = cmdstan_model(here("1_IGT_PP", "Code", "Stan", "Joint_ORL_Consistency.stan"), compile = F)
...
```{r, eval = T}
# Compile model
orl_model = stan_model(here("1_IGT_PP", "Code", "Stan", "Joint_ORL_Consistency.stan"))
...
saveRDS(orl_fit, here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_Consistency", "Joint_ORL_Consistency_fit.rds"))
```



```{r}
orl_fit = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_Consistency", "Joint_ORL_Consistency_fit.rds"))
...
saveRDS(orl_posteriors, here("1_IGT_PP", "Data", "2_Fitted", 
                             "Joint_ORL_Consistency", "Joint_ORL_Consistency_posteriors.rds"))
```
...
focal_parameters <- c(
  "mu_Arew", "mu_Apun", "mu_betaF", "mu_consistency", 
  "sigma_Arew", "sigma_Apun", "sigma_betaF", "sigma_c", 
  "Arew", "Apun", "betaF", "c",
  "R_Arew[1,2]", "R_Apun[1,2]", "R_betaF[1,2]", "R_c[1,2]"
)
...
saveRDS(rhats, here("1_IGT_PP", "Data", "2_Fitted", 
                    "Joint_ORL_Consistency", "Joint_ORL_Consistency_rhats.rds"))
```
...
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Joint_ORL_Consistency", "Joint_ORL_Consistency_group_traceplots.tiff"),
...
for(i in person_parameters){
  tiff(here("1_IGT_PP", "Figs_Tables", 
            "Joint_ORL_Consistency", "Joint_ORL_Consistency_", i, "_traceplots.tiff", sep = ""),
       width = 35, height = 25, units = "cm", res = 300)
    traceplot(orl_fit,
              pars = i
              ) %>% print()
...
```{r, eval = T}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Joint_ORL_Consistency", "Joint_ORL_Consistency_posterior_means.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Joint_ORL_Consistency", "Joint_ORL_Consistency_posterior_distributions.tiff"),
