...
combined_data = bind_rows(session1_data, session2_data) %>% 
  ...
  mutate(type = case_when(stim %in% c("C", "D") ~ "good", T ~ "bad")) %>% 
  group_by(Subject, session, type) %>% 
  reframe(choice_proportion = mean(play)) %>% 
  pivot_wider(names_from = c("type", "session"), values_from = "choice_proportion") %>% 
...
## Test-Retest
```{r}
cor.test(combined_data$good_1, combined_data$good_2)
cor.test(combined_data$bad_1, combined_data$bad_2)
```
