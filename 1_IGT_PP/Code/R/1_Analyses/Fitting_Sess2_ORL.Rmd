...
stan_data$Tsubj = stan_data$Tsubj[,2]
stan_data$choice = stan_data$choice[,,2]
stan_data$card = stan_data$card[,,2]
stan_data$outcome = stan_data$outcome[,,2]
stan_data$sign = stan_data$sign[,,2]
```
...
# Check Model
```{r, eval = F}
model_code = cmdstan_model(here("1_IGT_PP", "Code", "Stan", "ORL.stan"), compile = F)
...
```{r, eval = F}
# Compile model
orl_model = stan_model(here("1_IGT_PP", "Code", "Stan", "ORL.stan"))
...
saveRDS(orl_fit, here("1_IGT_PP", "Data", "2_Fitted", "Sess2_ORL", "Sess2_ORL_fit.rds"))
```



```{r}
orl_fit = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "Sess2_ORL", "Sess2_ORL_fit.rds"))
...
saveRDS(orl_posteriors, here("1_IGT_PP", "Data", "2_Fitted", 
                             "Sess2_ORL", "Sess2_ORL_posteriors.rds"))
```
...
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Sess2_ORL", "Sess2_ORL_group_traceplots.tiff"),
...
for(i in person_parameters){
  tiff(here("1_IGT_PP", "Figs_Tables", 
            "Sess2_ORL", "Sess2_ORL_", i, "_traceplots.tiff", sep = ""),
       width = 35, height = 25, units = "cm", res = 300)
    traceplot(orl_fit,
              pars = i
              ) %>% print()
...
```{r, eval = T}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Sess2_ORL", "Sess2_ORL_posterior_means.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Sess2_ORL", "Sess2_ORL_posterior_distributions.tiff"),
