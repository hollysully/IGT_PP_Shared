...
# Check Model
```{r, eval = T}
model_code = cmdstan_model(here("1_IGT_PP", "Code", "Stan", "Joint_ORL_start_noK.stan"), compile = F)
...
```{r, eval = T}
# Compile model
orl_model = stan_model(here("1_IGT_PP", "Code", "Stan", "Joint_ORL_start_noK.stan"))
...
saveRDS(orl_fit, here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_start_noK", "Joint_ORL_start_noK_fit.rds"))
```



```{r}
orl_fit = readRDS(here("1_IGT_PP", "Data", "2_Fitted", "Joint_ORL_start_noK", "Joint_ORL_start_noK_fit.rds"))
...
saveRDS(orl_posteriors, here("1_IGT_PP", "Data", "2_Fitted", 
                             "Joint_ORL_start_noK", "Joint_ORL_start_noK_posteriors.rds"))
```
...
focal_parameters <- c(
  ...
  "R_Arew[1,2]", "R_Apun[1,2]", "R_betaF[1,2]", "R_betaP[1,2]"
)
...
saveRDS(rhats, here("1_IGT_PP", "Data", "2_Fitted", 
                    "Joint_ORL_start_noK", "Joint_ORL_start_noK_rhats.rds"))
```
...
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Joint_ORL_start_noK", "Joint_ORL_start_noK_group_traceplots.tiff"),
...
for(i in person_parameters){
  tiff(here("1_IGT_PP", "Figs_Tables", 
            "Joint_ORL_start_noK", "Joint_ORL_start_noK_", i, "_traceplots.tiff", sep = ""),
       width = 35, height = 25, units = "cm", res = 300)
    traceplot(orl_fit,
              pars = i
              ) %>% print()
...
```{r, eval = T}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Joint_ORL_start_noK", "Joint_ORL_start_noK_posterior_means.tiff"),
     width = 35, height = 25, units = "cm", res = 300)
...
```{r}
tiff(here("1_IGT_PP", "Figs_Tables", 
          "Joint_ORL_start_noK", "Joint_ORL_start_noK_posterior_distributions.tiff"),
...
```{r}
data.frame(diff_Arew = orl_posteriors$mu_Arew[,2]-orl_posteriors$mu_Arew[,1],
           diff_Apun = orl_posteriors$mu_Apun[,2]-orl_posteriors$mu_Apun[,1],
           diff_betaP = orl_posteriors$mu_betaP[,2]-orl_posteriors$mu_betaP[,1],
           diff_betaF = orl_posteriors$mu_betaF[,2]-orl_posteriors$mu_betaF[,1]) %>% 
  pivot_longer(everything(), names_to = "parameter", values_to = "estimate", names_prefix = "diff_") %>% 
  group_by(parameter) %>% 
