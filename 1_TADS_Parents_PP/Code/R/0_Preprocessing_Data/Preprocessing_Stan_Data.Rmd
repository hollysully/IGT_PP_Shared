...
full = readRDS(here("1_TADS_Parents_PP", "Data", "0_Raw", "full_IGT.RDS"))
```
...
make_stan = function(data, covariate_names){
  ...
  for(i in 1:N){ # loop through persons
    ...
    for(s in 1:S){ # loop through sessions
      ...
      if(nrow(cur_is) > 0){ # check if participant has data - if not, skip next loop
  ...
  stan = list(ID_participant = ID_participant,
              ID_OG = ID_OG,
              ID_number = ID_number,
...
## Make Parent Data
```{r}
parent_stan = make_stan(data = bind_rows(T1_IGT_mom, T1_IGT_dad,
                                         T2_IGT_mom, T2_IGT_dad,
                                         T3_IGT_mom, T3_IGT_dad,
                                         T4_IGT_mom, T4_IGT_dad,
                                         T5_IGT_mom, T5_IGT_dad),
                        covariate_names = c())
saveRDS(parent_stan, here("1_TADS_Parents_PP", "Data", "1_Stan", "design_T12345_parent_IGT.RDS"))
```
