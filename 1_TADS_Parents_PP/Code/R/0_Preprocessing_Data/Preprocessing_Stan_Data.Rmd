...
combined_data = inner_join(IGT_prepped, SCID_prepped) %>% 
  # I'm removing this participant because they responded on < half of the trials and their estimates did not converge
  # filter(!(ID == 5183 & participant == "dad")) %>%
...
parent_stan = make_ORL_stan_data(data = bind_rows(combined_data),
                                 covariate_names = c("SU", "depression", "anxiety"))
                                 # covariate_names = c()) # no covariates
...
sample = "T1_parent_SU_Dep_Anx_Binomial"
