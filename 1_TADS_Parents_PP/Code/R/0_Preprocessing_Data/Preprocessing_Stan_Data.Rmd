...
# Inspect Data
```{r, eval = F}
bind_rows(T1_IGT_dad, T1_IGT_mom,
          T2_IGT_dad, T2_IGT_mom,
          T3_IGT_dad, T3_IGT_mom,
          T4_IGT_dad, T4_IGT_mom,
          T5_IGT_dad, T5_IGT_mom) %>% 
  filter(id >= 5000 & id < 6000) %>% 
  mutate(missing = case_when(card_RT == 4000 ~ 1, T ~ 0)) %>% 
  group_by(id, time, participant) %>% 
  summarise(`proportion trials missing` = mean(missing, na.rm = T)) %>% 
  View(title = "Missingness")
```
...
# covariates = c("anxiety", "depression", "SU")

IGT_prepped = bind_rows(
                        T1_IGT_dad, T1_IGT_mom,
                        T2_IGT_dad, T2_IGT_mom,
                        T3_IGT_dad, T3_IGT_mom,
                        T4_IGT_dad, T4_IGT_mom,
                        T5_IGT_dad, T5_IGT_mom
...
combined_data = inner_join(IGT_prepped, SCID_prepped) %>% 
  # exclude participants that responded on < 50% trials
  filter(!(ID == 5183 & participant == "dad" & time == "T1")) %>%
  filter(!(ID == 5064 & participant == "dad" & time == "T4")) %>%
  ...
  mutate(# fix session variable
         ...
         # create variable that assigns numbers to participant types (1 = child, 2 = mom, 3 = dad)
