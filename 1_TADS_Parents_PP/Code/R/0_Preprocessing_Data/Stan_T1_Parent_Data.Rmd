...
SCID = readRDS(here("1_TADS_Parents_PP", "Data", "0_Raw", "SCID_Anx_Dep_SUD.RDS")) # includes current diagnosis
```
...
sample = "T1_parent_current_cAnx_pAnx"
...
SCID_prepped = SCID %>%
  ...
  select(-date) %>% 
  # current diagnostics
  mutate(anxiety = case_when(canx == 1 ~ 1, canx == 0 & panx == 1 ~ 0)) %>% 
