...
group_parameters <- grep("mu|sigma|beta_", focal_parameters, value=TRUE)
person_parameters <- grep("mu|sigma|beta_", focal_parameters, value=TRUE, invert=TRUE)
```



## Load Data & Check Diagnostics
```{r}
orl_posteriors = list()
stan_datas = list()
...
for(s in 1:5) {
  # pull in data for session s - each of these gets saved because we'll use them later
  stan_datas[[s]] = readRDS(here("1_TADS_Parents_PP", "Data", "1_Stan",
                                 paste0("design_T", s, "_parent_IGT.RDS")))
  # pull in session s's model - this gets saved over each iteration, but we only need certain parts
  orl_fit = readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste0("T", s, "_parent_", model_name, "_fit.rds")))
  # pull in session s's posteriors - each of these gets saved because we'll use them later
  orl_posteriors[[s]] = readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                                     model_name,
                                     paste0("T", s, "_parent_", model_name, "_posteriors.rds")))
  # pull in session s's rhats and view them - this doesn't even get saved, it's just for checking
  readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
               model_name,
               paste0("T", s, "_parent_", model_name, "_rhats.rds"))) %>% 
    data.frame() %>% 
    View(title = paste0("Session_", s, "_Rhats"))
  
  # create traceplot of parameters for each session
  tiff(here("1_TADS_Parents_PP", "Figs_Tables",
            model_name, paste0("T", s, "_parent"),
