...
```{r}
# PROCESS POSTERIORS
processed_posteriors = data.frame(iteration = 1:((orl_fit@sim$iter - orl_fit@sim$warmup) * orl_fit@sim$chains),
...
diag_PPCs = 
  bind_rows(readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
    ...
    ungroup() %>% 
    mutate(group = factor(group, levels = c("Con", "Anx", "Dep", "SUD"), ordered = T))
```
...
## Prep Posterior Distribution Data
```{r, eval = F}
# ORGANIZE DATA
  ...
  orl_diag_parameters %>% 
      ggplot(aes(x = estimate, fill = group)) +
        ...
        geom_text_npc(data = horizontal_lab, aes(label = lab_),
                      npcx = "left", npcy = "top", size = 5, vjust = 1) +
        ...
        theme(#axis elements
              ...
              # plot elements
              plot.margin = unit(c(t = .75, r = .25, b = 0, l = .25), "cm"),
...
choice_pred = ifelse(orl_posteriors$choice_pred == 2, 0, 1)

session_wide_data = data.frame()
for(cur_sex in c("male", "female")){
  for(cur_card in c("A", "B", "C", "D")){
    cur_hdi = HDIofMCMC(apply(choice_pred[,sex_indices[[cur_sex]],card_indices[[cur_card]],], 1, mean))
    session_wide_data =
      data.frame(
        ID = stan_data$ID_number[sex_indices[[cur_sex]]],
        sex = cur_sex, 
        card = cur_card,
        obs = apply(ifelse(stan_data$choice == 2, 0, 1)[sex_indices[[cur_sex]],,], 1, mean),
        obs_mean = mean(apply(ifelse(stan_data$choice == 2, 0, 1)[sex_indices[[cur_sex]],card_indices[[cur_card]],], 1, mean)),
        pred = mean(apply(choice_pred[,sex_indices[[cur_sex]],card_indices[[cur_card]],], 1, mean)),
        lower = cur_hdi[1],
        upper = cur_hdi[2]) %>% 
...
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
  ...
  session_wide_data %>% 
    ggplot(aes(x = card, y = obs,
      ...
      geom_violin(position = position_dodge(width = .75), trim = F, alpha = .25) +
      ...
      geom_point(aes(y = pred), color = "black", shape = 21, fill = "white",
                 show.legend = F,
                 position = position_dodge(width = .75), alpha = .25) +
      scale_color_manual(values = c("goldenrod3", "dodgerblue2"),
                         labels = c("male" = "Male", "female" = "Female")) +
      scale_fill_manual(values = c("goldenrod3", "dodgerblue2"),
                        labels = c("male" = "Male", "female" = "Female")) +
