...
con_index = ind_parameters %>% 
  filter(con == 1, group == "con") %>% 
  group_by(ID) %>% 
  summarise(sum_abs_z = sum(abs(z))) %>% 
...
```{r, eval = F}
ind_parameters %>% 
    filter((ID %in% con_index & group == "con") |
           (ID %in% anx_index & group == "anx")) %>% 
  View(title = "Check Subseted Parameters")
```
...
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
     ...
     width = 7, height = 3.25, units = "cm", res = 300)

  # prep data
  ind_PPCs %>% 
    filter(ID %in% c(con_index, anx_index)) %>%
    mutate(group = case_when(ID %in% con_index ~ "Con",
                             ID %in% anx_index ~ "Anx"),
           grp_ind = "ind",
    ...
    mutate(trial = 1:30,
           observed = rollapply(choice, width = bin_width, FUN = mean,
                                na.rm = TRUE, fill = NA, partial = T)) %>% 
    bind_rows(mutate(grp_PPCs, grp_ind = "grp", pred = mu)) %>% 
    mutate(group = factor(group, levels = c("Con", "Anx", ordered = T))) %>% 
    filter(group %in% c("Con", "Anx"), deck == "B") %>% 
    # plotting
    ggplot(aes(x = trial, color = group)) +
      ...
      geom_point(aes(y = observed, fill = group), color = "black",
                 shape = 21, size = 1) +
      # scales
      scale_y_continuous(limits = c(.5,1),
                         breaks = seq(0,1,.25)) +
      scale_color_manual(values = c("black", "#E01818"), limits = c("Con", "Anx")) +
      scale_fill_manual(values = c("black", "#E01818"), limits = c("Con", "Anx")) +
      # theme
      labs(x = "Trial", y = "Proportion Play") +
      theme_classic() +
      ...
      guides(color = guide_legend(override.aes = list(size = 1.25), nrow = 1)) +
      facet_rep_wrap(.~grp_ind, scales = "free") +
      facetted_pos_scales(y = list(grp_ind == "grp" ~
                                     scale_y_continuous(limits = c(.5,1),
                                                        labels = c(".5", ".75", "1"),
                                                        breaks = seq(.5,1,.25)),
                                   grp_ind == "ind" ~
...
# -------------------------------------------
# Dep Beta BetaF Follow-up
## Frequency Plot
### Prep Data
```{r}
dep_data = ind_PPCs %>% 
  left_join(data.frame(ID = stan_data$ID_participant,
                       anx = stan_data$X[,2,],
                       dep = stan_data$X[,3,],
                       sud = stan_data$X[,4,])) %>% 
  filter(deck > 0) %>% 
  mutate(group = case_when(dep == 0 ~ "Con", T ~ "Dep"),
         deck = chartr("1234", "ABCD", as.character(deck))) %>%
  group_by(ID, anx, dep, sud, group, deck) %>% 
  summarise(prop = mean(choice)) %>% 
  filter(!(group == "Con" & anx+sud > 0))

# subset specific groups
con_dep_data = dep_data %>%
  filter(anx+dep+sud == 0) %>% 
  group_by(ID, deck) %>% 
  summarise(group = "Con", prop = mean(prop))

dep_full_data = dep_data %>%
  filter(dep == 1) %>% 
  group_by(ID, deck) %>% 
  summarise(group = "Dep", prop = mean(prop))

dep_only_data = dep_data %>%
  filter(dep == 1 & anx == 0 & sud == 0) %>% 
  group_by(ID, deck) %>% 
  summarise(group = "Dep", prop = mean(prop))

# calculate group-level means
con_dep_mus = con_dep_data %>% 
  group_by(group, deck) %>%
  summarise(prop = mean(prop)) %>%
  filter(group == "Con")

dep_only_mus = dep_only_data %>% 
  group_by(group, deck) %>%
  summarise(prop = mean(prop)) %>%
  filter(group == "Dep")

dep_full_mus = dep_data %>% 
  group_by(group, deck) %>%
  summarise(prop = mean(prop)) %>%
  filter(group == "Dep")
```
...
```{r, eval = F}
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
          model_name, sample, "Dep Beta BetaF Frequency Plot.tiff"),
     width = 7, height = 4.5, units = "cm", res = 300)

  # plotting
  ggplot(dep_data, aes(x = group, y = prop)) +
    # geoms
    ### control-group-related geoms ###
    geom_dotplot(data = con_dep_data, binaxis = "y", fill = "gray75",
                 stackdir = "center", stroke = .6, dotsize = 1.35) +
    geom_crossbar(data = con_dep_mus, aes(ymin = prop, ymax = prop),
                  linewidth = .5, fatten = 0, width = 1) +
    
    ### depression-related geoms ###
    ###### dep-only as red ######
    geom_dotplot(data = dep_full_data, binaxis = "y", fill = "gray75",
                 stackdir = "center", stroke = .6, dotsize = 1.25) +
    geom_dotplot(data = dep_only_data, binaxis = "y",
                 stackdir = "center", stroke = .6, dotsize = 1.25) +
    geom_crossbar(data = dep_full_mus, aes(ymin = prop, ymax = prop),
                  linewidth = .5, fatten = 0, width = 1) +
    geom_point(data = dep_only_mus, color = "red", size = .75) +
    ###### dep full as red ######
    # geom_dotplot(data = dep_full_data, binaxis = "y",
    #              stackdir = "center", stroke = .6, dotsize = 1.35) +
    # geom_dotplot(data = dep_only_data, binaxis = "y", fill = "gray85",
    #              stackdir = "center", stroke = .6, dotsize = 1.35) +
    # geom_crossbar(data = dep_full_mus, aes(ymin = prop, ymax = prop),
    #               linewidth = .5, fatten = 0, width = 1) +
    # geom_point(data = dep_only_mus, color = "red", size = .65) +
    
    ### other geoms ###
    geom_text_npc(aes(npcx = "right", npcy = "bottom", label = deck),
                  check_overlap = T) +
    geom_violin(scale = "count", fill = "transparent", trim = F, draw_quantiles = T) +
    # scales
    scale_y_continuous(limits = c(0, 1)) +
...
```{r}
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
          model_name, sample, "Dep Beta BetaF Person-Level PPCs.tiff"),
     width = 7, height = 3.25, units = "cm", res = 300)
  ...
  ind_PPCs %>% 
    ...
    mutate(trial = 1:30,
           observed = rollapply(choice, width = bin_width, FUN = mean,
                                na.rm = TRUE, fill = NA, partial = T)) %>% 
    bind_rows(mutate(grp_PPCs, grp_ind = "grp", pred = mu)) %>% 
    mutate(group = factor(group, levels = c("Con", "Dep", ordered = T))) %>% 
    filter(group %in% c("Con", "Dep"), deck %in% c("B", "D")) %>% 
    # plotting
    ggplot(aes(x = trial, color = group, linetype = deck)) +
      # geoms
      geom_ribbon(aes(ymin = lower95, ymax = upper95, fill = group), color = "transparent",
                  alpha = .2) +
      geom_ribbon(aes(ymin = lower50, ymax = upper50, fill = group), color = "transparent",
                  alpha = .375) +
      geom_line(aes(y = pred), linewidth = .5) +
      geom_point(aes(y = observed, fill = group, shape = deck), color = "black",
                 size = 1) +
      # scales
      scale_y_continuous(limits = c(.5,1),
                         breaks = seq(0,1,.25)) +
      scale_color_manual(values = c("black", "#2184DA"), limits = c("Con", "Dep")) +
      scale_fill_manual(values = c("black", "#2184DA"), limits = c("Con", "Dep")) +
