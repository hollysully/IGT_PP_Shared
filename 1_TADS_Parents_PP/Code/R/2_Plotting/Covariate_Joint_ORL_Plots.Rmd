  ...
  orl_diag_parameters %>% 
      ggplot(aes(x = estimate, fill = group)) +
        ...
        scale_color_manual(values = c("black", "#E01818", "#2184DA", "#FFC107"),
                           limits = c("Comparison", "Anx", "Dep", "SUD")) +
        ...
        theme(#axis elements
              ...
              # plot elements
              plot.margin = unit(c(t = 1.1, r = .25, b = 0, l = .25), "cm"),
...
data.frame(stan_data$X[,2:4,1]) %>% 
  ...
  mutate(Nothing     = case_when(Anx == 0 & Dep == 0 & SUD == 0 ~ 1, T ~ 0),
         Anx_Dep     = case_when(Anx == 1 & Dep == 1 ~ 1, T ~ 0),
         Anx_SUD     = case_when(Anx == 1 & SUD == 1 ~ 1, T ~ 0),
         Dep_SUD     = case_when(Dep == 1 & SUD == 1 ~ 1, T ~ 0),
