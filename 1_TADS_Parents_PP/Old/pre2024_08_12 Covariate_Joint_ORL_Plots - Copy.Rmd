...
grp_PPCs = 
  bind_rows(readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_Con.rds", sep = "_"))),
            readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_Anx.rds", sep = "_"))),
            readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_Dep.rds", sep = "_"))),
            readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_SUD.rds", sep = "_")))) %>% 
...
## Choice Proportions
```{r}
big_loss_IDs = data.frame(ID = stan_data$ID_participant, 
                  card = stan_data$card, 
                  outcome = stan_data$outcome,
                  choice = stan_data$choice) %>%
  pivot_longer(cols = c(starts_with("card"), starts_with("outcome"), starts_with("choice")),
               names_to = c("type", "trial"),
               values_to = "dv", names_sep = "\\.",
               names_transform = list(trial = as.numeric)) %>% 
  pivot_wider(names_from = "type", values_from = "dv") %>% 
  mutate(choice = ifelse(choice == 1, 1, 0)) %>% 
  group_by(ID, card) %>% 
  arrange(trial) %>% 
  mutate(card_trial = rank(trial),
         consec_trial = cumsum(choice)) %>%
  filter(outcome == -11.5 & consec_trial == 9) %>% 
  ungroup() %>% 
  select(ID)
```
...
```{r}
temp = left_join(ind_PPCs, diagnosis) %>%
  # right_join(big_loss_IDs) %>%
  group_by(ID, deck, anx) %>%
  mutate(trial = rank(trial)) %>%
  ungroup() %>% 
  
  group_by(trial, deck, anx) %>% 
  summarise(pred = mean(pred),
            choice = mean(choice)) %>% 
  ungroup() %>% arrange(trial) %>% 
  group_by(deck, anx) %>% 
  mutate(choice = rollapply(choice, width = bin_width, FUN = mean,
                              na.rm = TRUE, fill = NA, partial = T),
...
temp %>% 
  filter(deck == 2) %>% 
  mutate(deck = "B",
         group = case_when(anx == 0 ~ "Anx-", anx == 1 ~ "Anx+")) %>% 
  # plotting
  ggplot(aes(x = trial, color = group)) +
    ...
    geom_text_npc(data = data.frame(comparison = 1, deck = "B", lab_ = "Deck B"),
                  ...
                  size = 3, hjust = 0, vjust = .75) +
    geom_point(aes(y = choice), position = position_jitter(width = .5, height = 0)) +
...
## Cumulative Choices
```{r}
temp2 = left_join(ind_PPCs, diagnosis) %>%
  group_by(ID, deck, anx) %>%
  mutate(trial = rank(trial)) %>%
  ungroup() %>%
  ...
  group_by(ID, deck, anx) %>% 
  arrange(trial) %>% 
  mutate(choice = cumsum(choice)) %>%
  ungroup() %>% 
  group_by(deck, anx, trial) %>% 
  summarise(choice = r2spss::trimmed_mean(choice, trim = .0))
...
temp2 %>% 
  ...
  ggplot(aes(x = trial, color = group)) +
    ...
    # geom_ribbon(aes(ymin = lower50, ymax = upper50, fill = group), color = "transparent",
    #             alpha = .375) +
    geom_line(aes(y = choice), linewidth = .5, alpha = .25) +
    # geom_point(aes(y = observed, fill = group),
    #            size = .75, shape = 21, color = "black") +
    # scales
    # scale_y_continuous(limits = c(.5,1),
...
# subset individual participants
...
anx_index = ind_parameters %>% 
  ...
  # filter(anx == 1, group == "anx", dep == 0, sud == 0) %>%
  group_by(ID) %>% 
  summarise(sum_abs_z = sum(abs(z))) %>% 
  arrange(sum_abs_z) %>% head(1) %>% .$ID
...
anx_comparison2 = 
  bind_rows(readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "Anx_Only_PPCs.rds", sep = "_"))),
            readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "Con_PPCs.rds", sep = "_")))) %>%
  mutate(ID = "group", comparison = 2,
         group = case_when(group == "Con" ~ "Anx-", group == "Anx_Only" ~ "Anx+")) %>% 
  select(pred = mu, everything())
...
anx_comparison3 = 
  ind_PPCs %>% 
    ...
    mutate(ID = "ind", comparison = 3, observed = choice)

Anx_PPCs = bind_rows(anx_comparison1, anx_comparison2, anx_comparison3) %>% 
  group_by(ID, comparison, trial, deck, group) %>% 
  summarise(observed = mean(observed), pred = mean(pred),
            lower50 = mean(lower50), upper50 = mean(upper50),
            lower95 = mean(lower95), upper95 = mean(upper95)) %>%
  ungroup() %>% group_by(deck, group, comparison) %>% 
  # calculate rolling averages
  mutate(observed = rollapply(observed, width = bin_width, FUN = mean,
                              na.rm = TRUE, fill = NA, partial = T),
         pred = rollapply(pred, width = bin_width, FUN = mean,
...
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
     ...
     width = 7, height = 3, units = "cm", res = 300)
  ...
  Anx_PPCs %>% 
    ...
    ggplot(aes(x = trial, color = group)) +
      ...
      facetted_pos_scales(y = list(comparison != 3 ~
                                     scale_y_continuous(limits = c(.5,1),
                                                        labels = c(".5", ".75", "1"),
                                                        breaks = seq(.5,1,.25)),
                                   comparison == 3 ~
...
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
  ...
  Anx_PPCs %>% 
    ...
    ggplot(aes(x = trial, color = group)) +
      # geoms
      geom_text_npc(data = data.frame(comparison = 1, deck = c("B", "D"), lab_ = c("Deck B", "Deck D")),
                    aes(npcx = "left", npcy = "top", label = lab_),
                    size = 3, hjust = 0, vjust = .75) +
      geom_ribbon(aes(ymin = lower95, ymax = upper95, fill = group), color = "transparent",
                  alpha = .2) +
...
dep_index = ind_parameters %>% 
  # filter(dep == 1) %>%
  filter(dep == 1, group == "dep", anx == 0, sud == 0) %>%
  group_by(ID) %>% 
  summarise(sum_abs_z = sum(abs(z))) %>% 
  arrange(sum_abs_z) %>% head(1) %>% .$ID
...
dep_comparison1 = 
  bind_rows(readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
            ...
            readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_Con_Dep.rds", sep = "_")))) %>%
  mutate(ID = "group", comparison = 1,
         group = case_when(group == "Con_Dep" ~ "Dep-", group == "Dep" ~ "Dep+")) %>% 
  select(pred = mu, everything())

dep_comparison2 = 
  bind_rows(readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_Dep_Only.rds", sep = "_"))),
            readRDS(here("1_TADS_Parents_PP", "Data", "2_Fitted",
                         model_name,
                         paste(sample, model_name, "PPCs_Con.rds", sep = "_")))) %>%
  mutate(ID = "group", comparison = 2,
         group = case_when(group == "Con" ~ "Dep-", group == "Dep_Only" ~ "Dep+")) %>% 
  select(pred = mu, everything())

dep_comparison3 = 
  ind_PPCs %>% 
    filter(ID %in% c(con_index, dep_index)) %>%
    mutate(group = case_when(ID %in% con_index ~ "Dep-",
                             ID %in% dep_index ~ "Dep+"),
           deck = chartr("1234", "ABCD", as.character(deck))) %>% 
    group_by(ID, group, deck) %>%
    arrange(trial) %>% 
...
Dep_PPCs = bind_rows(dep_comparison1, dep_comparison2, dep_comparison3) %>% 
  group_by(ID, comparison, trial, deck, group) %>% 
  summarise(observed = mean(observed), pred = mean(pred),
            lower50 = mean(lower50), upper50 = mean(upper50),
            lower95 = mean(lower95), upper95 = mean(upper95)) %>%
...
tiff(here("1_TADS_Parents_PP", "Figs_Tables",
  ...
  Dep_PPCs %>% 
    ...
    ggplot(aes(x = trial, color = group)) +
      ...
      scale_y_continuous(limits = c(.5,1),
                         breaks = seq(0,1,.25)) +
      scale_color_manual(values = c("black", "#2184DA"), limits = c("Dep-", "Dep+")) +
      scale_fill_manual(values = c("gray25", "#2184DA"), limits = c("Dep-", "Dep+")) +
      # theme
      labs(x = "Trial", y = "Proportion Play") +
      theme_classic() +
      theme(legend.title = element_blank(),
