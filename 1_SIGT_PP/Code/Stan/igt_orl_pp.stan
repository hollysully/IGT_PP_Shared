...
// Declare transformed parameters
  ...
  // Calculate & transform subject-level parameters to use in RL algorithm
  for(i in 1:N){
    Arew[i] = Phi_approx(mu_p[1] + Arew_pr[i]*sigma_Arew);
    Apun[i] = Phi_approx(mu_p[2] + Apun_pr[i]*sigma_Apun);
    K[i]    = Phi_approx(mu_p[3] + K_pr[i]*sigma_K) * 5;
    betaF[i] = mu_p[4] + betaF_pr[i]*sigma_betaF;
    betaP[i] = mu_p[5] + betaP_pr[i]*sigma_betaP;
  }
}
