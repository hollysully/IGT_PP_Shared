...
## Prep Data
```{r}
orders = read_excel(here("1_SIGT_PP", "Data", "0_Raw", "S-IGT Log.xlsx")) %>% 
  select(ID, first = `First?`) %>% 
  mutate(SIGT_first = case_when(first == "IGT" ~ 0, first == "S-IGT" ~ 1))
...
combined_data = inner_join(questionnaire_data, IGT_data) %>% 
  select(ID, SIGT_first, bastot, basdrive, basfunsk, basrewres, bis,
